...
The GoogleSamples repo demonstrates another flavor of the MVP architecture where view logic is implemented in
the Fragment. The interesting part of this pattern is having the View and Presenter interfaces defined in a
Contract interface.
...
I like the concept of a Presenter contract, but have implemented in a slightly diffent location in the PresenterActivity
pattern. Each View class defines an interface which must be implemented by the caller. The interface allows the
Presenter to responds to events generated by the View and can be used in conjunction with the View public methods 
to drive the View.  

This pattern is demonstrated in code samples in the next section. 
